...
void TimedIncrementT_Gen_T_Sr0_T_Sr1_W(bool setAverage, int t_Gen = 0,
		int t_Sr0 = 0, int t_Sr1 = 0, float w = 0.0f);
...
int Haw0_k = 0, Haw1_k = 0; //Shirina impulsa na polovine amplitudy   *0.1 mkS //pri kalibrovke
...
unsigned int DacOffs = 0; //DAC offset unsigned
int DacScale = 4095; //DAC scale unsigned
int PotVal = 0x40; //Digital potentiometer value
float W = 0, AA = 0, BB = 0, a0 = 0, a1 = 0, a2 = 0, a3 = 0, a4 = 0, a5 = 0,
		a6 = 0, a7 = 0, a8 = 0, a9 = 0, k1 = 0, Wmin = 0, Wmax = 0;
...
static const char * abc_ru = { "������ ��������������������������1234567890" };
...

{
	...
	W = AA
			+ BB
					* (a0 + a1 * (Am0 - Am0_k) * 10
							+ a2 * (Haw0 - Haw0_k) / 1000
							+ a3 * (Gamma0 - Gamma0_k) / 1000
							+ a4 * (T_Gen - T_Gen_k) / 100
							+ a5 * (T_Sr0 - T_Sr0_k) / 100);
	printf("W=");
	printf("%f", W);
	printf(" \n");
...

{
	...
	Kalibr: byte PosX = 0xA;
	byte PosY = 0x10;

	SetCursor(0xA, 0x10);
	Key = 0x0;
	if (VO != 0) {	//SetCursor(0xA,0xF);
		...
		do {
			...
			if (tempchar == '1' || tempchar == '3' || tempchar == '5'
					|| tempchar == '7' || tempchar == '9') {
				Key = tempchar;
			};
			if (IS_EXPERIMENT && Key == '1' || Key == '3' || Key == '7') {
				goto Exk;
			};
...

{
	...
	Rab: if (VO != 0) {	//SetCursor(0xA,0xF);
		...
		do {
			...
			if (IS_EXPERIMENT && Key == '3' || Key == '5' || Key == '7') {
				goto Exr;
			}
		} while ((xtim / 1000 - xtim_n / 1000) < VO);
		Rel = Rel ^ 0x01;
...

{
	...
	Grad: if (VO != 0) {
		...
		do {
			xtim = clock();
			tempchar = ReadKeyBuffered();
			if (tempchar == '1' || tempchar == '5' || tempchar == '7'
					|| tempchar == '9') {
				Key = tempchar;
			};
			if (IS_EXPERIMENT && Key == '1' || Key == '5' || Key == '7') {
				goto Exg;
			};
	...
	tempchar = ReadKeyBuffered();
	if (tempchar == '9' || tempchar == '5' || tempchar == '7'
			|| tempchar == '1') {
		Key = tempchar;
	};
...

{
	...
	do {
		...
		if (tempchar == '9' || tempchar == '5' || tempchar == '3'
		...
		if (Key == '9') {
			...
			PosY++;
			PosY++;
			VZ = (temp[PosY][0] - 48) * 100 + (temp[PosY][1] - 48) * 10
					+ temp[PosY][2] - 48;
			PosY++;
			VO = (temp[PosY][0] - 48) * 100 + (temp[PosY][1] - 48) * 10
					+ temp[PosY][2] - 48;
			PosY++;
			VI = (temp[PosY][0] - 48) * 100 + (temp[PosY][1] - 48) * 10
					+ temp[PosY][2] - 48;
			PosY++;
			Tak = (temp[PosY][0] - 48) * 100 + (temp[PosY][1] - 48) * 10
					+ temp[PosY][2] - 48;
			PosY++;
...
char PeriphSetup(void) {
	...
	do {
		...
		if (Key == '9') {
			PosY = 0;
			DacOffs = 1000 * (temp[PosY][0] - 48) + 100 * (temp[PosY][1] - 48)
					+ 10 * (temp[PosY][2] - 48) + (temp[PosY][3] - 48);
			PosY++;
			DacScale = 1000 * (temp[PosY][0] - 48) + 100 * (temp[PosY][1] - 48)
					+ 10 * (temp[PosY][2] - 48) + (temp[PosY][3] - 48);
			PosY++;
			PotVal = 1000 * (temp[PosY][0] - 48) + 100 * (temp[PosY][1] - 48)
					+ 10 * (temp[PosY][2] - 48) + (temp[PosY][3] - 48);
...
char Porog() // Regim slugebnojn nastrojki
{
	...
	do {
		tempchar = ReadKeyBuffered();
		if (tempchar == '5' || tempchar == '9' || tempchar == 'C'
				|| tempchar == '2' || tempchar == '8' || tempchar == 'S') {
			Key = tempchar;
		};
		...
		if (Key == '9') {
			...
			Poralfa = (temp[PosY][0] - 48) * 1000 + (temp[PosY][1] - 48) * 100
					+ (temp[PosY][2] - 48) * 10 + (temp[PosY][3] - 48)
					+ (temp[PosY][5] - 48) * 0.1 + (temp[PosY][6] - 48) * 0.01
					+ (temp[PosY][7] - 48) * 0.001;
			PosY++;
			Porbeta = (temp[PosY][0] - 48) * 1000 + (temp[PosY][1] - 48) * 100
					+ (temp[PosY][2] - 48) * 10 + (temp[PosY][3] - 48)
					+ (temp[PosY][5] - 48) * 0.1 + (temp[PosY][6] - 48) * 0.01
					+ (temp[PosY][7] - 48) * 0.001;
			PosY++;
			Porgamma = (temp[PosY][0] - 48) * 1000 + (temp[PosY][1] - 48) * 100
					+ (temp[PosY][2] - 48) * 10 + (temp[PosY][3] - 48)
					+ (temp[PosY][5] - 48) * 0.1 + (temp[PosY][6] - 48) * 0.01
					+ (temp[PosY][7] - 48) * 0.001;
			PosY++;
		...
		if (Key == '7') {
			PosY = 0;
			Saw_A0 = (temp[PosY][0] - 48) * 1000000
					+ (temp[PosY][1] - 48) * 100000
					+ (temp[PosY][2] - 48) * 10000 + (temp[PosY][3] - 48) * 1000
					+ (temp[PosY][5] - 48) * 100 + (temp[PosY][6] - 48) * 10
					+ (temp[PosY][7] - 48) * 1;
			PosY++;

			//printf("Saw_A1=");printf("%u",Saw_A1);printf(" \n");
			Saw_A1 = (temp[PosY][0] - 48) * 1000000
					+ (temp[PosY][1] - 48) * 100000
					+ (temp[PosY][2] - 48) * 10000 + (temp[PosY][3] - 48) * 1000
					+ (temp[PosY][5] - 48) * 100 + (temp[PosY][6] - 48) * 10
					+ (temp[PosY][7] - 48) * 1;
...

{
	...
	do {
		...
		if (Key == '9') {
			...
			PosY++;
			PosY++;
			Va = (temp[PosY][0] - 48) * 1000 + (temp[PosY][1] - 48) * 100
					+ (temp[PosY][2] - 48) * 10 + (temp[PosY][3] - 48)
					+ (temp[PosY][5] - 48) * 0.1 + (temp[PosY][6] - 48) * 0.01
					+ (temp[PosY][7] - 48) * 0.001;
			PosY++;
			kgt = (44 - temp[PosY][0])
					* ((temp[PosY][1] - 48) * 100 + (temp[PosY][2] - 48) * 10
							+ (temp[PosY][3] - 48) + (temp[PosY][5] - 48) * 0.1
							+ (temp[PosY][6] - 48) * 0.01
							+ (temp[PosY][7] - 48) * 0.001);
			PosY++;
			...
			T = (temp[PosY][0] - 48) * 1000 + (temp[PosY][1] - 48) * 100
					+ (temp[PosY][2] - 48) * 10 + (temp[PosY][3] - 48)
					+ (temp[PosY][5] - 48) * 0.1 + (temp[PosY][6] - 48) * 0.01
					+ (temp[PosY][7] - 48) * 0.001;
			PosY++;
...

{
	...
	do {
		tempchar = ReadKeyBuffered();
		if (tempchar == '9' || tempchar == '5' || tempchar == 'C'
				|| tempchar == '2' || tempchar == '8' || tempchar == 'S') {
			Key = tempchar;
		};
		...
		if (Key == '9') {
			...
			N_Rez = (temp[PosY][0] - 48) * 1000 + (temp[PosY][1] - 48) * 100
					...
					+ (temp[PosY][7] - 48) * 0.001;
			PosY++;
			K_Rez = (temp[PosY][0] - 48) * 1000 + (temp[PosY][1] - 48) * 100
					+ (temp[PosY][2] - 48) * 10 + (temp[PosY][3] - 48)
					+ (temp[PosY][5] - 48) * 0.1 + (temp[PosY][6] - 48) * 0.01
					+ (temp[PosY][7] - 48) * 0.001;
			PosY++;
			Step_Rez = (temp[PosY][0] - 48) * 1000 + (temp[PosY][1] - 48) * 100
					+ (temp[PosY][2] - 48) * 10 + (temp[PosY][3] - 48)
					+ (temp[PosY][5] - 48) * 0.1 + (temp[PosY][6] - 48) * 0.01
					+ (temp[PosY][7] - 48) * 0.001;
			PosY++;
...

{
	...
	do {
		tempchar = ReadKeyBuffered();
		if (tempchar == '9' || tempchar == 'C' || tempchar == '2'
				|| tempchar == '8' || tempchar == 'S') {
			Key = tempchar;
		};
...

{
	...
	printf("%x", tempchar);
	printf(" \n");
	//strcpy(Text,"��������:  ������:  "); tempchar=PrintXY(0,3,20);printf("%x",tempchar);printf(" \n");
...
char PrintXY(byte X, byte Y, byte length) // Pechat stroki simvolov
	...
	ComDataWrite[pos] = 0x02;
	pos++;              //byte 0
	ComDataWrite[pos] = 0x68;
	pos++;             //byte 1
	ComDataWrite[pos] = X;
	pos++;                //byte 2
	ComDataWrite[pos] = Y;
...
char ComWriteIntReg(char UstrAdr, char StartAdr, char Length) // Zapis registrov
	...
	ComDataWrite[pos] = UstrAdr;
	pos++;           //byte 0
	ComDataWrite[pos] = 0x45;
	pos++;             //byte 1
	ComDataWrite[pos] = 0x00;
	pos++;              //byte 2
	ComDataWrite[pos] = StartAdr;
	pos++;          //byte 3

	if (UstrAdr == 1) {
		for (i = 0; i < Length; i++) {
			ComDataWrite[pos] = floor(
					(float) RegistrSPM_int[StartAdr + i] / 256);
			pos++;
	...
	if (UstrAdr == 2) {
		for (i = 0; i < Length; i++) {
			ComDataWrite[pos] = floor(
					(float) RegistrTM_int[StartAdr - 0x30 + i] / 256);
			pos++;
			ComDataWrite[pos] = fmod((float) RegistrTM_int[StartAdr - 0x30 + i],
					256);
			pos++;
		};
	};
...
char ComReadIntReg(char UstrAdr, char StartAdr, char Length) // Chtenie registrov
	...
	ComDataWrite[pos] = UstrAdr;
	pos++;           //byte 0
	ComDataWrite[pos] = 0x44;
	pos++;             //byte 1
	ComDataWrite[pos] = 0x00;
	pos++;              //byte 2
	ComDataWrite[pos] = StartAdr;
	pos++;          //byte 3
	ComDataWrite[pos] = Length;
	pos++;            //byte 4
	...
	if (UstrAdr == 1) {
		for (i = 0; i < Length; i++) {
			RegistrSPM_int[StartAdr + i] = ComDataRead[i * 2 + 2] * 256
					+ ComDataRead[i * 2 + 3];
	...
	if (UstrAdr == 2) {
		for (i = 0; i < Length; i++) {
			RegistrTM_int[StartAdr + i] = ComDataRead[i * 2 + 2] * 256
					+ ComDataRead[i * 2 + 3];
			// printf("%u",StartAdr+i);printf(" = ");
...
char ComCMode(char UstrAdr, char UstrFuncArgument) // Ustanovka regima raboti
	...
	ComDataWrite[pos] = UstrAdr;
	pos++;           //byte 0
	ComDataWrite[pos] = 0x42;
	pos++;             //byte 1
	ComDataWrite[pos] = UstrFuncArgument;
	pos++;  //byte 2
...
char Reader(bool restore) //ctenie faylov nastroek
	...
	char gradPath[256];
	strcpy(gradPath,
			CombinePath(applicationPath,
					restore ? "Grad.dat" : GetNumberedFileName(Grad)));
	char nastrPath[256];
	strcpy(nastrPath,
			CombinePath(applicationPath,
					restore ? "Nastr.dat" : GetNumberedFileName(Nastr)));
	char kalibrPath[256];
	strcpy(kalibrPath,
			CombinePath(applicationPath,
					restore ? "Kalibr.dat" : GetNumberedFileName(Kalibr)));
	char setPath[256];
	strcpy(setPath,
			CombinePath(applicationPath,
					restore ? "Set.dat" : GetNumberedFileName(Set)));
	//printf("Read");printf(" \n");
	FILE *fpg;
...
char WriteUBI() //display temperature and humidity on remote indicator
{
	...
	// Podgotovka komandnoj stroki             //Write:
	ComDataWrite[pos] = 0x03;
	pos++; 	          //select indicator
	ComDataWrite[pos] = 0x10;
	pos++;   	          //write multiple registers
	ComDataWrite[pos] = 0x00;
	pos++;              //
	ComDataWrite[pos] = 0x00;
	pos++;          	 //starting adress
...
void TimedIncrementT_Gen_T_Sr0_T_Sr1_W(bool setAverage, int t_Gen, int t_Sr0,
		int t_Sr1, float w) {
...
bool CheckFilling() {
	printf("Haw0 = %i", Haw0);
	bool filling = Nzp == 0
			|| !(Nzp == 1 && Am0 > Poralfa) && !(Nzp == 2 && Am0 < Poralfa)
...
bool ShutdownServer() {
	...
	OpenProcessToken(hdlProcessHandle, TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY,
			&hdlTokenHandle);
	...
	AdjustTokenPrivileges(hdlTokenHandle, false, &tkp, sizeof(TOKEN_PRIVILEGES),
...
char * GetFullRow(char * buffer) {
	int bufferlength =
			strlen(buffer) < ROW_WIDTH - 1 ? strlen(buffer) : ROW_WIDTH - 1;
...
bool HasSomeDefaultFiles() {
	...
	return !stat(CombinePath(applicationPath, "Grad.dat"),
			(struct stat *) &fileinfo)
			|| !stat(CombinePath(applicationPath, "Nastr.dat"),
					(struct stat *) &fileinfo)
			|| !stat(CombinePath(applicationPath, "Kalibr.dat"),
					(struct stat *) &fileinfo)
			|| !stat(CombinePath(applicationPath, "Set.dat"),
					(struct stat *) &fileinfo);
}
...
void SaveKalibr() {
	char * kalibrPath = CombinePath(applicationPath,
			GetNumberedFileName(Kalibr));
	FILE *fp;
	fp = fopen(kalibrPath, "w+b");
...
char ReaderOld() //ctenie faylov nastroek
{
	...
	strcpy(gradPath,
			CombinePath(applicationPath,
					GetTranslatedFileName(Grad, Trim(mediumName))));
	char nastrPath[256];
	strcpy(nastrPath,
			CombinePath(applicationPath,
					GetTranslatedFileName(Nastr, Trim(mediumName))));
	char kalibrPath[256];
	strcpy(kalibrPath,
			CombinePath(applicationPath,
					GetTranslatedFileName(Kalibr, Trim(mediumName))));
	char setPath[256];
	strcpy(setPath,
			CombinePath(applicationPath,
					GetTranslatedFileName(Set, Trim(mediumName))));
	FILE *fpg;
	fpg = fopen(gradPath, "r+b");
	if (fpg != NULL) {
		fread(&a0, sizeof(float), 1, fpg);
...
char * GetTranslatedFileName(FileType type, char * mediumName) {
	...
	int length =
			strlen(rootstr) + strlen(mediumstr) + strlen(mediumstr) > 0 ?
					strlen("_.dat") : strlen(".dat");
...
char * GetTranslatedString(char * source) {
	...
	for (int i = 0; i < initlength; i++) {
		if (i < initlength - 1 && source[i] == '�' && source[i + 1] == '�') {
		...
		if (source[i] >= '�' && source[i] <= '�') {
			strcat(translated, notation[(int) source[i] - (int) '�']);
		} else {
